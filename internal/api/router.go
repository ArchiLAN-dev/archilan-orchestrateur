...
func NewRouter(cfg *config.Config, svc *service.Service) http.Handler {
	...
	r.Group(func(r chi.Router) {
		...
		r.Put("/apworlds/{hash}/yaml", handleSetApworldTemplate(svc))
		r.Post("/apworlds/{hash}/template", handleRegenerateApworldTemplate(svc))
		r.Post("/apworlds/{hash}/introspect", handleReintrospectApworldOptions(svc))
